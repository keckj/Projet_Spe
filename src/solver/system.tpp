...
template <typename T>
System<T>::System(std::map<std::string, Grid<T> *> *initialCond) :
	grids(0), grids_old(0), _dh(0.0)
{
	...
	grids_old = initialCond;

	grids = new std::map<std::string, Grid<T> *>;
	
	for(auto it = initialCond->begin(); it != initialCond->end(); it++) {
		(*grids)[it->first] = it->second->clone();			
		(*grids)[it->first]->allocateOnCpu();
	}
}
